            ...
            async for res in result_generator:
                ...
                for output in res.outputs:
                    ...
                    if output.finish_reason is None:
                        ...
                        if len(res.metrics.every_enqueue_time) > 2:
                            assert all([t<tpost for t, tpost in zip(res.metrics.every_enqueue_time[:-1], res.metrics.every_enqueue_time[1:])])
                        if len(res.metrics.every_deque_time) > 2:
                            assert all([t<tpost for t, tpost in zip(res.metrics.every_deque_time[:-1], res.metrics.every_deque_time[1:])])
                        final_usage = UsageInfo(
                            prompt_tokens=prompt_tokens,
                            completion_tokens=previous_num_tokens[i],
                            total_tokens=prompt_tokens +
